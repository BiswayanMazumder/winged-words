...
export default function Otherusertweetsui(props) {
  ...
  const ownerid = localStorage.getItem('userid');
    ...
    const fetchtweets = async (lastVisibleTweet) => {
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          const userDocRef = doc(db, "User Uploaded Tweet ID", ownerid); //change UID here for tweets from other users
    ...
    const getcoverpics = () => {
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          const uid = user.uid;
          const userDocRef = doc(db, "Cover Picture", ownerid);//change UID here for cover pic from other users
          const docSnap = await getDoc(userDocRef);
          if (docSnap.exists()) {
    ...
    const getprofilepics = async () => {
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          const uid = user.uid;
          const userDocRef = doc(db, "User Details", ownerid);////change UID here for profile pic from other users
          const docSnap = await getDoc(userDocRef);
          if (docSnap.exists()) {
